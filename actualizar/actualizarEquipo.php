...
$id = $_GET['id'] ?? null;
$equipo_actual = Equipo::obtenerEquipoPorID($conexion, $id);
...
<body class="bg-gray-100">
    <main class="mx-auto max-w-3xl px-4 py-10">
        <header class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-4xl font-bold text-gray-800">Actualizar Equipo</h1>
                <p class="mt-2 text-lg text-gray-600">Completa los campos para actualizar el equipo en el inventario.</p>
            </div>
        ...
        <section class="rounded-xl bg-white p-6 shadow-md sm:p-8">
            <form action="../objetos/agregarEquipo.php" method="post" class="space-y-6">
                ...
                <input type="hidden" name="equipo_id" value="<?php echo htmlspecialchars($equipo_actual['equipo_id'] ?? ''); ?>">
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                   
                    <div>
                        <label for="txt_noSerie" class="mb-2 block text-sm font-medium text-gray-700">Número de Serie</label>
                        <input type="text" id="txt_noSerie" name="txt_noSerie" required 
                               value="<?php echo htmlspecialchars($equipo_actual['no_serie'] ?? ''); ?>" 
                        ...
                        <div class="relative">
                            ...
                            <?php foreach($lst_marcas as $marca): ?>
                                <option value="<?php echo($marca["marca_id"]) ?>" 
                                    <?php if(($marca["marca_id"] == ($equipo_actual["marca_id"] ?? ''))) echo 'selected'; ?>>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="txt_descripcion" name="txt_descripcion" rows="3" required class="block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-gray-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200"><?php echo htmlspecialchars($equipo_actual['descripcion'] ?? ''); ?></textarea>
                    </div>

                    <div>
                        <label for="date_fechaCompra" class="mb-2 block text-sm font-medium text-gray-700">Fecha de Compra</label>
                        <input type="date" name="date_fechaCompra" id="date_fechaCompra" required 
                               value="<?php echo htmlspecialchars($equipo_actual['fecha_compra'] ?? ''); ?>" 
                               class="block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-gray-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200">
                    </div>

                    <div>
                        <label for="txt_precio" class="mb-2 block text-sm font-medium text-gray-700">Precio</label>
                        <input type="number" step="0.01" name="txt_precio" id="txt_precio" required 
                               value="<?php echo htmlspecialchars($equipo_actual['precio'] ?? ''); ?>" 
                               class="block w-full rounded-lg border border-gray-300 px-3 py-2.5 text-gray-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200">
                    </div>
                        ...
                        <div class="relative">
                            ...
                            <?php foreach($lst_tipos as $tipo): ?>
                                <option value="<?php echo($tipo["tipo_id"]) ?>" 
                                    <?php if(($tipo["tipo_id"] == ($equipo_actual["tipo_equipo"] ?? ''))) echo 'selected'; ?>>
                        ...
                        <div class="relative">
                            ...
                            <?php foreach($lst_empleados as $empleado): ?>
                                <option value="<?php echo($empleado["empleado_id"]) ?>" 
                                    <?php if(($empleado["empleado_id"] == ($equipo_actual["empleado_id"] ?? ''))) echo 'selected'; ?>>
                                    <?php echo($empleado["nombreCompleto"]) ?>
                                </option>
                            <?php endforeach; ?>
                            </select>
                            <svg class="pointer-events-none absolute right-3 top-1/2 h-5 w-5 -translate-y-1/2 text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" aria-hidden="true">
                ...
                <div class="flex justify-end gap-3 border-t border-gray-200 pt-6">
                    <a href="../equipos.php" class="rounded-lg bg-gray-200 px-4 py-2 font-bold text-gray-700 hover:bg-gray-300">Cancelar</a>
